...
Stage 2d's motif samplers (`gopcnet.simulation.sample_chain`,
`sample_overlapping_triangles`), and the generic screening/composition
primitives (`gopcnet.screening.compute_pairwise_screening_evidence`,
`screen_uncorrected`, `gopcnet.pipeline.compose_screen_then_prune`). The
...
from gopcnet.simulation import sample_chain, sample_overlapping_triangles
...
def run_stage2j(config: Stage2jConfig, output_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    from gopcnet.experiments.stage2j_reporting import select_alpha_p10
