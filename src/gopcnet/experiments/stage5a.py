...
Runs MINT's conservative engine (`gopcnet.pipeline.
compose_screen_then_prune`) and a from-specification EBICglasso
re-implementation (`gopcnet.comparators.ebicglasso.fit_ebicglasso`) on
...
p=15 network (`gopcnet.experiments.stage4l`), the composed, noisy
shared-node overlap p=15 network (`gopcnet.experiments.stage2d`), and
the three-node balanced/moderate/strong triangle fixtures
(`gopcnet.simulation.motifs`). No new DGP, no new MINT engine code, no
...
from gopcnet.experiments import stage2d, stage4l
...
from gopcnet.simulation.motifs import sample_precision_triangle
...
def run_stage5a(
    ...
    if not write_report:
        return raw
    from gopcnet.experiments.stage5a_reporting import write_stage5a_report
