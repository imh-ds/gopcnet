...
| Composed pipeline at **`p=30`** — screening (`alpha=.0001`, D-023) then DPI (`alpha=f(N)`, D-012), disjoint chain/fork/triangle motifs | `N in [750, 1500]` (both PROCEED). Final false-edge rate `.000146`/`.000101`, identical to screening-alone's own rate at both `N` (D-014's "DPI cannot rescue an isolated false positive" finding replicated at `p=30`); indirect TPR `.838`/`.891`; true-edge FPR `<=.006`; triad-formation rate `.983`/`.994` (slightly higher than `p=15`'s `~.96`) | Validated only for disjoint 3-node motifs at `p=30` — hub, overlap, or other candidate shapes at `p=30` remain untested (would each need their own charter, mirroring Stage 2c/2d after Stage 2b at `p=15`); bootstrap stability remains untested at `p=30` (Stage 3's line of work is `p=15`-only); not tested beyond `p=30` | Stage 2f, `docs/decision_log.md` D-024 |
