...
| Composed pipeline at **`p=30`**, **shared-node-overlap shape** — same mechanism, same D-023 screening threshold | **Located floor: `N=1750`, not `1500`.** `N in {1500, 1600}`: REASSESS (overlap TPR `.762`/`.786`, both below the `.80` gate — `1500` is `.750` at `p=15` (D-018) but no longer sufficient at `p=30`). **`N >= 1750`: PROCEED** (TPR `.815` at `1750`, rising to `.906` by `2500`). Chain/fork TPR and true-edge FPR behave normally at every tested `N`; the effect is isolated to the overlap motif's weak (`~.135`) cross-branch signal meeting `p=30`'s stricter, signal-agnostic threshold | Floor is specific to this exact signal strength (`~.135`) and D-023's exact threshold (`alpha=.0001`) — a different weak signal or a different `p`-driven threshold needs its own floor search, not an assumption this transfers. Do not use `N=1500` or `1600` for this shape at `p=30` | Stage 2h/2i, `docs/decision_log.md` D-026, D-027 |
