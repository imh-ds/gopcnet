...
| Composed pipeline — screening (`alpha=.001`) then generalized DPI (`mintnet.pipeline.compose`) within any validated clique shape (3-node triad or 4-node hub-clique), using the D-012 `alpha(N)` formula (`p=15`) | `N in [750, 1500]` (both PROCEED, tested both on a triad-only network, D-014, and on a network mixing triads and a 4-node hub at once, D-016). Final false-edge rate exactly tracks screening's own rate in every test so far; true-edge FPR is `0` in the mixed-shape network, `~.005` in the triad-only network; shape-validated rate `~.96`-`.97` in both | **Only validated for disjoint, non-overlapping candidate components of size 3 or 4 that are full cliques.** Non-clique shapes, cliques of any other size, larger networks, and components sharing variables across motifs are explicitly untested — passed through unmodified by design, not oversight | Stage 2b/2c, `docs/decision_log.md` D-014, D-016 |
| Multi-variable conditioning — DPI conditioning on all other nodes in a candidate component, tested on a 4-node hub (1 hub, 3 children) | `N in [750, 1500]` (both PROCEED, comfortable margin `.05`-`.09`). The existing D-012 `alpha(N)` formula works unmodified — no new fit needed for two-variable conditioning. **Now wired into the composed pipeline** and confirmed working there too (D-016) | Validated only for the hub shape (one shared cause, independent children). Components formed by two motifs sharing a node, or larger structures, remain untested | Stage 1k, `docs/decision_log.md` D-015 |
| Multi-variable conditioning — same rule, tested on a genuinely different topology: two `balanced` triangles sharing one node (5 variables, 3-variable conditioning) | `N in [750, 1500]` (both PROCEED, margin `.06`-`.09`). Same D-012 `alpha(N)` formula, unmodified, now confirmed across two distinct topologies (hub and shared-node overlap) | **Mechanism only — not wired into the composed pipeline.** Unlike the hub case, screening's power to detect this DGP's weak (`~.135`) cross-branch correlation is only `~66%` at `N=750` (vs. `~98%` at `N=1500`), so a clean 5-node candidate clique will frequently not form at `N=750` — a full-pipeline wiring charter would need to separately characterize that detection rate, not assume it | Stage 1L, `docs/decision_log.md` D-017 |
