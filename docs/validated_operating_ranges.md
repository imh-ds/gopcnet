...
| Composed pipeline — screening (`alpha=.001`) then DPI within clean 3-node candidate triads, using the D-012 `alpha(N)` formula (`p=15`, disjoint chain/fork/triangle motifs) | `N in [750, 1500]` (both PROCEED). Final false-edge rate exactly tracks screening's own rate (DPI essentially never rescues an isolated false positive); indirect-edge TPR (`.818`/`.861`) is slightly below DPI-alone's isolated numbers, explained by the ~4% of replicates where a motif's candidate component wasn't a clean triad | **Only validated for disjoint, non-overlapping 3-node candidate components.** Overlapping motifs, hub variables, or candidate components larger than 3 nodes are explicitly untested — DPI is not applied to them at all in the current mechanism, by charter design, not oversight | Stage 2b, `docs/decision_log.md` D-014 |
| Multi-variable conditioning — DPI conditioning on all other nodes in a candidate component (extends the mechanism beyond exact 3-node triads), tested on a 4-node hub (1 hub, 3 children) | `N in [750, 1500]` (both PROCEED, comfortable margin `.05`-`.09`). The existing D-012 `alpha(N)` formula works unmodified — no new fit needed for two-variable conditioning | Validated only for the hub shape (one shared cause, independent children) — **not yet wired into the composed pipeline** (`mintnet.pipeline.compose` still restricts DPI to exact triads). Components formed by two motifs sharing a node, or larger structures, remain untested | Stage 1k, `docs/decision_log.md` D-015 |
