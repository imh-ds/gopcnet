...
| Bootstrap edge stability — row resampling (`B=500`) of the composed screen-then-prune pipeline, final-edge stability `pi_final` thresholded at a calibrated `pi_min` | `N in [750, 1500]` (both PROCEED), on **two** structurally different DGPs now: disjoint chain/fork/triangle (D-014) and chain/fork/hub (D-016), `p=15` each. Smallest candidate `pi_min=.70` eligible and selected at both `N` on **both** DGPs; stability recall `.995`-`1.0`, stability FDR `0`-`.007`, no measurable regression vs. the point-estimate baseline | Validated only for these two composed shapes, both already known to compose cleanly at the point estimate (D-014, D-016) — **not** validated as this general selection gate for the shared-node-overlap DGP (addressed separately, and only for filtering, below), nor for `B` values other than `500`. Descriptive-only evidence on the overlap DGP (D-019) shows wrongly-retained edges land at *intermediate* stability (`~.53` pooled; see the corrected conditional split below), clearly above null (`~.02`) but below true edges (`~1.0`) | Stage 3/3c, `docs/decision_log.md` D-019, D-021 |
